...
import {
  defaultSentinelIgnores,
  isIgnoredPath,
  SentinelFinding,
  SentinelHandle,
  startFilesystemSentinel,
} from "../src/sentinel/filesystem";
import { AuditEvent } from "../src/types";

/**
 * What the filesystem sentinel reports, what it refuses to report, and what it leaves behind.
 *
 ...
 * in the sentinel's internal debounce, which a faked clock would advance past before the real
 ...
 * signal that the sentinel has worked through the earlier event, so "nothing was reported"
...
// the copy would leave the sentinel calling the untouched original - and the degradation test
...
const openHandles: SentinelHandle[] = [];
const auditEvents: AuditEvent[] = [];

function tempDir(): string {
  const dir = nodeFs.mkdtempSync(join(tmpdir(), "agentwall-fs-sentinel-"));
  tempDirs.push(dir);
  return dir;
}
...
class FindingCollector {
  ...
  private readonly waiters: Array<{ match: (f: SentinelFinding) => boolean; deliver: (f: SentinelFinding) => void }> = [];

  readonly onFinding = (finding: SentinelFinding): void => {
  ...
  waitFor(match: (f: SentinelFinding) => boolean): Promise<SentinelFinding> {
    ...
    return new Promise<SentinelFinding>((resolve) => {
...
async function startSentinel(
  ...
  const handle = await startFilesystemSentinel({
    paths,
...
describe("filesystem sentinel findings", () => {
  ...
  it("stays quiet for a benign file", async () => {
    const dir = tempDir();
    const collector = new FindingCollector();
    const handle = await startSentinel([dir], collector);
  ...
  it("records the finding in the audit chain with the path and types but not the contents", async () => {
    ...
    const event = auditEvents.find((candidate) => candidate.action === "fs:secret-written");
    if (event === undefined) throw new Error("the sentinel produced no audit record for the finding");

    expect(event.plane).toBe("content");
    expect(event.decision).toBe("deny");
    expect(event.riskLevel).toBe("high");
    expect(event.matchedRules).toContain("content:deny-fs-secret-write");
    ...
    const fsDetection = detections.find((detection) => detection.id === "det.content.fs.secret_written");
...
describe("filesystem sentinel candidate filtering", () => {
  it("skips a file larger than maxFileBytes instead of reading it", async () => {
    const dir = tempDir();
    const collector = new FindingCollector();
    const handle = await startSentinel([dir], collector, { maxFileBytes: 64 });
  ...
  it("applies segment patterns and suffix patterns as documented, and adds to the defaults", () => {
    ...
    expect(isIgnoredPath("/w/pkg/.git/config", defaultSentinelIgnores)).toBe(true);
    expect(isIgnoredPath("/w/node_modules/lib/index.js", defaultSentinelIgnores)).toBe(true);
    expect(isIgnoredPath("/w/assets/logo.png", defaultSentinelIgnores)).toBe(true);

    // `.env` is the file this whole module exists for; no default may swallow it.
    expect(isIgnoredPath("/w/.env", defaultSentinelIgnores)).toBe(false);
    expect(isIgnoredPath("/w/src/config.ts", defaultSentinelIgnores)).toBe(false);

    // A caller's ignores extend the defaults; they never replace them.
    const extended = [...defaultSentinelIgnores, "fixtures/", "*.snap"];
...
describe("filesystem sentinel debouncing and races", () => {
  it("collapses a burst of writes to one path into a single scan", async () => {
    const dir = tempDir();
    const collector = new FindingCollector();
    const handle = await startSentinel([dir], collector, { debounceMs: 250 });
    ...
    for (let attempt = 0; attempt < 20; attempt++) {
      ...
      // Twenty writes inside a single tick would collapse into one scan even from a sentinel
...
describe("filesystem sentinel shutdown", () => {
  it("closes every watcher and reports nothing after close resolves", async () => {
    ...
    const handle = await startSentinel([dir], first);
    ...
    // A second sentinel is the clock: once IT has reported a write to this directory, a live
    // watcher on the first sentinel would have reported the earlier one too.
    const second = new FindingCollector();
    await startSentinel([dir], second);
...
describe("filesystem sentinel degradation", () => {
  ...
  //
  //   npx jest tests/kill-switch.test.ts tests/scan-api.test.ts \
  //            tests/mcp-http.test.ts tests/fs-sentinel.test.ts --maxWorkers=1
  //
  ...
  it("falls back to periodic re-scan when the operator asks for it", async () => {
    const dir = tempDir();
    const collector = new FindingCollector();
    const handle = await startSentinel([dir], collector, { mode: "rescan", rescanIntervalMs: 50, debounceMs: 20 });
  ...
  it("falls back and declares itself degraded when recursive watch is unavailable", async () => {
    ...
    const handle = await startSentinel([dir], collector, { rescanIntervalMs: 50, debounceMs: 20 });
  ...
  it("refuses to start on an unusable path and leaves no watcher behind", async () => {
    ...
    await expect(
      startFilesystemSentinel({ paths: [join(dir, "absent")], onFinding: collector.onFinding }),
    ).rejects.toThrow(/cannot watch/);
    await expect(startFilesystemSentinel({ paths: [], onFinding: collector.onFinding })).rejects.toThrow(
      /at least one path/,
    );
    ...
    await expect(
      startFilesystemSentinel({ paths: [dir, join(dir, "absent")], onFinding: collector.onFinding }),
    ).rejects.toThrow(/cannot watch/);
    expect(fsEventWrapCount()).toBe(baselineWatchers);
  });
});
