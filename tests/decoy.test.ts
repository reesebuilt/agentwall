...
import type { DecoyKind } from "../src/decoy";
import {
  DECOY_KINDS,
  decoyEnvBlock,
  clearDecoys,
...
/**
 * The decoy plane's two load-bearing claims, and nothing else.
 *
 ...
 */
const SHAPE_BY_KIND: Record<DecoyKind, RegExp> = {
  "aws-access-key": /^AKIA[0-9A-Z]{16}$/,
  "github-pat": /^ghp_[A-Za-z0-9]{36}$/,
  "openai-key": /^sk-[A-Za-z0-9]{48}$/,
  "generic-secret": /^[0-9a-f]{64}$/,
  url: /^https:\/\/api-[0-9a-f]{12}\.decoy\.invalid\/v1\/collect\/[0-9a-f]{32}$/,
};
...
function tempFile(name: string): string {
  const dir = mkdtempSync(join(tmpdir(), "agentwall-decoy-"));
  tempDirs.push(dir);
...
describe("decoy minting", () => {
  ...
  it("mints an AWS decoy a real AWS key id regex would accept", () => {
    const token = generateDecoy("aws-access-key", "billing-decoy");
  ...
  it("mints a GitHub decoy whose trailing checksum is a real CRC-32 of its body", () => {
    ...
    const token = generateDecoy("github-pat");
  ...
  it("never repeats a value across repeated calls", () => {
    ...
    for (const kind of DECOY_KINDS) {
      for (let i = 0; i < 200; i += 1) {
        values.add(generateDecoy(kind).value);
        minted += 1;
      }
    }
  ...
  it("gives every token a distinct id", () => {
    const ids = new Set(Array.from({ length: 200 }, () => generateDecoy("openai-key").id));
    expect(ids.size).toBe(200);
  });
});

describe("decoy matching", () => {
  it("finds a registered token embedded in a URL query string", () => {
    const token = generateDecoy("aws-access-key", "billing-decoy");
    registerDecoy(token);

    const hits = scanForDecoys(`GET https://drop.example.net/collect?key=${token.value}&seq=4 HTTP/1.1`);
    expect(hits).toEqual([token]);
  });

  it("finds a registered token embedded in a JSON body", () => {
    const token = generateDecoy("openai-key");
    registerDecoy(token);

    const body = JSON.stringify({ note: "environment dump", env: { OPENAI_API_KEY: token.value, PATH: "/usr/bin" } });
    expect(scanForDecoys(body)).toEqual([token]);
  });
  ...
  it("returns nothing for an unregistered value of the same shape", () => {
    ...
    registerDecoy(generateDecoy("aws-access-key"));

    const lookalike = generateDecoy("aws-access-key");
    expect(scanForDecoys(`AWS_ACCESS_KEY_ID=${lookalike.value}`)).toEqual([]);
    // The key id AWS itself uses in documentation: correct shape, not our decoy, not a hit.
    expect(scanForDecoys("AWS_ACCESS_KEY_ID=AKIAIOSFODNN7EXAMPLE")).toEqual([]);
    expect(scanForDecoys("nothing interesting in this payload at all")).toEqual([]);
  });

  it("does not fire on a truncated or altered copy of the decoy", () => {
    const token = generateDecoy("generic-secret");
    registerDecoy(token);

    expect(scanForDecoys(token.value.slice(0, -1))).toEqual([]);
    expect(scanForDecoys(`${token.value.slice(0, -1)}z`)).toEqual([]);
  });
  ...
  it("reports one hit even when the value appears several times", () => {
    ...
    expect(scanForDecoys(`${token.value} ... ${token.value}`)).toEqual([token]);
    expect(events).toHaveLength(1);
  });

  it("matches an isolated candidate through the constant-time path", () => {
    const token = generateDecoy("aws-access-key");
    const other = generateDecoy("aws-access-key");
    registerDecoy(token);

    expect(matchesDecoyValue(token.value)).toEqual(token);
    expect(matchesDecoyValue(other.value)).toBeNull();
    // Length mismatch must be a miss, not a throw: timingSafeEqual rejects unequal buffers.
    expect(matchesDecoyValue("short")).toBeNull();
    expect(matchesDecoyValue(`${token.value}xx`)).toBeNull();
  });

  it("refuses to register a value short enough to collide with ordinary traffic", () => {
    const token = { ...generateDecoy("aws-access-key"), value: "AKIA1" };
    expect(() => registerDecoy(token)).toThrow(/minimum is 16/);
  });
});

describe("decoy audit records", () => {
  it("records a hit as a critical identity deny with the ATT&CK mapping attached", () => {
    const token = generateDecoy("aws-access-key", "billing-decoy");
    registerDecoy(token);
    const events = captureAuditEvents();

    scanForDecoys(`key=${token.value}`, { agentId: "agent-7", sessionId: "sess-3", surface: "probe-api" });
    ...
    expect(events[0]).toMatchObject({
      ...
      agentId: "agent-7",
      sessionId: "sess-3",
      matchedRules: ["identity:deny-decoy-triggered"],
    });
    expect(events[0].metadata).toMatchObject({
      decoyId: token.id,
      decoyKind: "aws-access-key",
      decoyLabel: "billing-decoy",
      decoyTriggered: "true",
      decoySurface: "probe-api",
    });
  ...
  it("keeps the decoy value out of the serialized record", () => {
    ...
    const token = generateDecoy("github-pat", "ci-decoy");
    registerDecoy(token);
    const events = captureAuditEvents();

    scanForDecoys(`Authorization: Bearer ${token.value}`);
...
describe("decoy files", () => {
  it("writes the decoy file at mode 0600 and reads it back intact", () => {
    const file = tempFile("decoys.json");
    const tokens = [generateDecoy("aws-access-key", "billing-decoy"), generateDecoy("url")];

    saveDecoys(file, tokens);

    expect(statSync(file).mode & 0o777).toBe(0o600);
    expect(loadDecoys(file)).toEqual(tokens);
  });
  ...
  it("restores 0600 when saving over a file somebody loosened", () => {
    ...
    const file = tempFile("decoys.json");
    saveDecoys(file, [generateDecoy("openai-key")]);
  ...
  it("refuses a group- or world-readable decoy file and says why", () => {
    const file = tempFile("decoys.json");
    saveDecoys(file, [generateDecoy("aws-access-key")]);
    chmodSync(file, 0o644);

    expect(() => loadDecoys(file)).toThrow(/mode 0644/);
    expect(() => loadDecoys(file)).toThrow(/strip them from outbound traffic/);
    expect(() => loadDecoys(file)).toThrow(/chmod 600/);
  });
  ...
  it("rejects a file whose entries do not survive the token schema", () => {
    ...
    saveDecoys(file, [{ ...generateDecoy("aws-access-key"), value: "tiny" }]);

    expect(() => loadDecoys(file)).toThrow(/not a valid token list/);
  });
});

describe("decoy env block", () => {
  it("emits export lines a shell accepts and reproduces the values exactly", () => {
    const tokens = DECOY_KINDS.map((kind) => generateDecoy(kind));
    const block = decoyEnvBlock(tokens);
  ...
  it("gives same-kind decoys distinct variable names so none is overwritten", () => {
    const labelled = generateDecoy("aws-access-key", "prod runner");
    const first = generateDecoy("aws-access-key");
    const second = generateDecoy("aws-access-key");

    const block = decoyEnvBlock([labelled, first, second]);
    const names = block
      .split("\n")
      .filter((line) => line.startsWith("export "))
  ...
  it("labels the block as decoy material so nobody treats it as a real credential dump", () => {
    const block = decoyEnvBlock([generateDecoy("generic-secret", "db-decoy")]);
...
describe("decoy CLI", () => {
  it("generate prints the token with a plantable export block and saves it", () => {
    ...
    runDecoyCommand(["generate", "--kind", "github-pat", "--label", "ci-decoy", "--out", file]);

    const output = logSpy.mock.calls.map((call) => call.join(" ")).join("\n");
    const saved = loadDecoys(file);
  ...
  it("generate appends to an existing decoy file rather than replacing it", () => {
    const file = tempFile("decoys.json");
    jest.spyOn(console, "log").mockImplementation(() => undefined);

    runDecoyCommand(["generate", "--kind", "aws-access-key", "--out", file]);
    runDecoyCommand(["generate", "--kind", "openai-key", "--out", file]);

    expect(loadDecoys(file).map((token) => token.kind)).toEqual(["aws-access-key", "openai-key"]);
  });
  ...
  it("list shows the inventory and never prints a value", () => {
    ...
    runDecoyCommand(["list", "--file", file]);
  ...
  it("rejects an unknown kind and an unknown subcommand instead of guessing", () => {
    expect(() => runDecoyCommand(["generate", "--kind", "ssh-key"])).toThrow(/Unknown decoy kind "ssh-key"/);
    expect(() => runDecoyCommand(["inspect", "--file", "x"])).toThrow(/Unknown decoy subcommand "inspect"/);
    expect(() => runDecoyCommand(["generate", "--kind"])).toThrow(/--kind needs a value/);
    expect(() => runDecoyCommand(["list"])).toThrow(/decoy list needs --file/);
  });
});
